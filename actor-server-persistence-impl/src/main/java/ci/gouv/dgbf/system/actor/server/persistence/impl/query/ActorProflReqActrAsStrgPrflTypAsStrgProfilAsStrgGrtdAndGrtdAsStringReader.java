...
public class ActorProflReqActrAsStrgPrflTypAsStrgProfilAsStrgGrtdAndGrtdAsStringReader extends AbstractActorProfileRequestReaderImpl implements Serializable {
	...
	protected String getQueryValue() {
		...
		arguments.getProjection(Boolean.TRUE).addFromTuple("pt", ProfileType.FIELD_NAME);
		arguments.getTuple(Boolean.TRUE).add("ActorProfileRequest t").addJoins("JOIN Actor a ON a = t.actor","JOIN Profile p ON p = t.profile","JOIN ProfileType pt ON pt = p.type");
	...
	protected void __set__(ActorProfileRequest actorProfileRequest, Object[] array) {
		...
		actorProfileRequest.setProfileTypeAsString(getAsString(array, index++));
	}
}