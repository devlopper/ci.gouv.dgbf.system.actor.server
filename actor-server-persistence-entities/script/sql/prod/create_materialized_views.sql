...
-- Exercice
DROP MATERIALIZED VIEW "ACTEUR".VM_APP_EXERCICE;
CREATE MATERIALIZED VIEW "ACTEUR".VM_APP_EXERCICE
TABLESPACE USERS
REFRESH NEXT SYSDATE + 1/24 COMPLETE AS
...
-- PAYS
DROP MATERIALIZED VIEW "ACTEUR".VM_APP_PAYS;
CREATE MATERIALIZED VIEW "ACTEUR".VM_APP_PAYS
TABLESPACE USERS
REFRESH NEXT SYSDATE + 1/24 COMPLETE AS
SELECT DISTINCT t.id_pays AS "IDENTIFIANT",t.code_pays AS "CODE",t.libelle_pays AS "LIBELLE"
FROM UT_BIDF_TAMP.t_pays@dblink_elabo_bidf t ORDER BY t.libelle_pays DESC;
