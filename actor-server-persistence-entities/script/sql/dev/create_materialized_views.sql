...
-- Service
DROP MATERIALIZED VIEW "SIIBC_ACTEUR".VM_APP_SERVICE;
CREATE MATERIALIZED VIEW "SIIBC_ACTEUR".VM_APP_SERVICE
REFRESH ON COMMIT COMPLETE AS
...
FROM SIIBC_PRT.msvc_service t,SIIBC_PRT.msvc_module m,SIIBC_PRT.msvc_menu me
...
-- Menu
DROP MATERIALIZED VIEW "SIIBC_ACTEUR".VM_APP_MENU;
CREATE MATERIALIZED VIEW "SIIBC_ACTEUR".VM_APP_MENU
REFRESH ON COMMIT 
COMPLETE
AS
SELECT
...
-- Privil�ge
DROP MATERIALIZED VIEW "SIIBC_ACTEUR".VM_APP_PRIVILEGE;
CREATE MATERIALIZED VIEW "SIIBC_ACTEUR".VM_APP_PRIVILEGE
--REFRESH ON COMMIT 
...
UNION ALL SELECT identifiant,code,libelle,'CATEGORIE_BUDGET' AS "TYPE" FROM SIIBC_ACTEUR.VM_APP_CAT_BUDGET
...
DROP MATERIALIZED VIEW "SIIBC_ACTEUR".VM_APP_PAYS;
CREATE MATERIALIZED VIEW "SIIBC_ACTEUR".VM_APP_PAYS
TABLESPACE USERS
REFRESH NEXT SYSDATE + 1/24 COMPLETE AS
SELECT DISTINCT t.id_pays AS "IDENTIFIANT",t.code_pays AS "CODE",t.libelle_pays AS "LIBELLE"
FROM t_pays@dblink_elabo_bidf t ORDER BY t.libelle_pays DESC;
ALTER TABLE VM_APP_PAYS ADD CONSTRAINT VM_APP_PAYS_PK PRIMARY KEY (IDENTIFIANT);
CREATE INDEX VM_APP_PAYS_K_CODE ON VM_APP_PAYS (CODE DESC);
...
-- Poste , mail et demande
DROP MATERIALIZED VIEW VMA_POSTE_EMAIL_DEMANDE;
CREATE MATERIALIZED VIEW VMA_POSTE_EMAIL_DEMANDE
TABLESPACE USERS
REFRESH NEXT SYSDATE + 1/24 COMPLETE AS
SELECT pe.identifiant||pd.identifiant||d.identifiant AS "IDENTIFIANT",p.identifiant AS "POSTE",pe.email AS "EMAIL",d.identifiant AS "DEMANDE"
FROM VA_POSTE_EMAIL pe
JOIN Poste p ON p.code = pe.poste_code
JOIN DM_POSTE pd ON pd.poste = p.identifiant AND pd.accordee = 1
JOIN DM_DEMANDE d ON d.email = pe.email AND d.statut = 'ACCEPTEE'
GROUP BY pe.identifiant||pd.identifiant||d.identifiant,p.identifiant,pe.email,d.identifiant;
ALTER TABLE VMA_POSTE_UTILISE ADD CONSTRAINT VMA_POSTE_UTILISE_PK PRIMARY KEY (IDENTIFIANT);
CREATE INDEX VMA_POSTE_UTILISE_K_POSTE ON VMA_POSTE_UTILISE (POSTE ASC);
CREATE INDEX VMA_POSTE_UTILISE_K_EMAIL ON VMA_POSTE_UTILISE (EMAIL ASC);
CREATE INDEX VMA_POSTE_UTILISE_K_DEMANDE ON VMA_POSTE_UTILISE (DEMANDE ASC);