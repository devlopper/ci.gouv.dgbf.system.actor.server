...
AS SELECT
    ...
    ,s.secb_code||' '||s.secb_libelle AS "section"
    ,d.signataire_acte_nomination AS "signataire_acte_nomination"
    ,UPPER('FICHE DE RENSEIGNEMENT ET DE DEPÔT DE SIGNATURE DES '||(CASE SUBSTR(p.code,1,1) WHEN 'G' THEN 'GESTIONNAIRES DE CRéDITS' WHEN 'O' THEN 'ORDONNATEURS' ELSE 'XXX' END)) AS "titre"
    ,g.libelle AS "type_utilisateur"
    ,ua.ua_code||' '||ua.ua_liblg AS "unite_administrative"
