...
@Queries(value = {
	@org.cyk.utility.persistence.annotation.Query(tupleClass = Section.class,name = SectionQuerier.QUERY_NAME_READ_ALL,value = "SELECT t FROM Section t ORDER BY t.code ASC")
})
public interface SectionQuerier extends Querier.CodableAndNamable<Section> {

	String PARAMETER_NAME_SEARCH = "search";
	
	String QUERY_IDENTIFIER_READ_DYNAMIC = QueryIdentifierBuilder.getInstance().build(Section.class, QueryName.READ_DYNAMIC);	
	String QUERY_IDENTIFIER_READ_DYNAMIC_ONE = QueryIdentifierBuilder.getInstance().build(Section.class, QueryName.READ_DYNAMIC_ONE);
	String QUERY_IDENTIFIER_COUNT_DYNAMIC = QueryIdentifierBuilder.getInstance().build(Section.class, QueryName.COUNT_DYNAMIC);
	...
	public static abstract class AbstractImpl extends Querier.CodableAndNamable.AbstractImpl<Section> implements SectionQuerier,Serializable {
		...
		@Override
		public Section readOne(QueryExecutorArguments arguments) {
			if(QUERY_IDENTIFIER_READ_DYNAMIC_ONE.equals(arguments.getQuery().getIdentifier()))
				return DynamicOneExecutor.getInstance().read(Section.class,arguments.setQuery(null));
		...
		public Collection<Section> readMany(QueryExecutorArguments arguments) {
			if(QUERY_IDENTIFIER_READ_DYNAMIC.equals(arguments.getQuery().getIdentifier()))
				return DynamicManyExecutor.getInstance().read(Section.class,arguments.setQuery(null));
		...
		public Long count(QueryExecutorArguments arguments) {
			if(QUERY_IDENTIFIER_COUNT_DYNAMIC.equals(arguments.getQuery().getIdentifier()))
				return DynamicManyExecutor.getInstance().count(Section.class,arguments.setQuery(null));
