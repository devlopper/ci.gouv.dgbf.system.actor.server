...
public interface ScopeOfTypeSectionQuerier extends Querier {
	...
	static String getQueryValueReadVisibleSectionsWhereFilterWherePredicateVisible() {
		return "(EXISTS (SELECT actorScope.identifier FROM ActorScope actorScope "
				+ " WHERE actorScope.actor.code = :"+PARAMETER_NAME_ACTOR_CODE+" AND actorScope.scope.identifier = scope.identifier) "
				+ " OR "
				...
				+ " OR "
				+ " EXISTS (SELECT actorScope.identifier FROM ActorScope actorScope "
				+ " JOIN Scope scopeBudgetSpecializationUnit ON actorScope.scope = scopeBudgetSpecializationUnit JOIN BudgetSpecializationUnit budgetSpecializationUnit ON budgetSpecializationUnit = scopeBudgetSpecializationUnit "
				+ " WHERE actorScope.actor.code = :"+PARAMETER_NAME_ACTOR_CODE+" AND budgetSpecializationUnit.section = scope)"
				+ ")";
	}
	
	static String getQueryValueReadVisibleSectionsWhereFilterWherePredicate() {
		return Where.and("scope.type.code = '"+ScopeType.CODE_SECTION+"'"
				,getQueryValueReadVisibleSectionsWhereFilterWherePredicateVisible(),Where.like("scope", Scope.FIELD_CODE, PARAMETER_NAME_CODE)
				,Where.like("scope", Scope.FIELD_NAME, PARAMETER_NAME_NAME, NUMBER_OF_WORDS_OF_PARAMETER_NAME_NAME));
	}
	
	static String getQueryValueReadVisibleSectionsWhereFilterWhere() {
		return Where.of(getQueryValueReadVisibleSectionsWhereFilterWherePredicate());
	}
	...
	static String getQueryValueReadInvisibleSectionsWhereFilterWhere() {
		return "WHERE "+Language.Where.and("scope.type.code = '"+ScopeType.CODE_SECTION+"'"
				,"NOT "+getQueryValueReadVisibleSectionsWhereFilterWherePredicateVisible(),Language.Where.like("scope", Scope.FIELD_CODE, PARAMETER_NAME_CODE)
	...
	public static abstract class AbstractImpl extends AbstractObject implements ScopeOfTypeSectionQuerier,Serializable {
		...
		public Collection<Scope> readMany(QueryExecutorArguments arguments) {
			if(QUERY_IDENTIFIER_READ_VISIBLE_SECTIONS_WHERE_FILTER.equals(arguments.getQuery().getIdentifier()))
				return readVisibleSectionsWhereFilter(arguments);
			if(QUERY_IDENTIFIER_READ_INVISIBLE_SECTIONS_WHERE_FILTER.equals(arguments.getQuery().getIdentifier()))
				return readInvisibleSectionsWhereFilter(arguments);			
			throw new RuntimeException("Query <<"+arguments.getQuery().getIdentifier()+">> not readable by "+getClass());
		}
		
		@Override
		public Long count(QueryExecutorArguments arguments) {
			if(QUERY_IDENTIFIER_COUNT_VISIBLE_SECTIONS_WHERE_FILTER.equals(arguments.getQuery().getIdentifier()))
				return countVisibleSectionsWhereFilter(arguments);
			if(QUERY_IDENTIFIER_COUNT_INVISIBLE_SECTIONS_WHERE_FILTER.equals(arguments.getQuery().getIdentifier()))
				return countInvisibleSectionsWhereFilter(arguments);			
		...
		public Collection<Scope> readVisibleSectionsWhereFilter(QueryExecutorArguments arguments) {
			if(arguments.getQuery() == null)
				arguments.setQueryFromIdentifier(QUERY_IDENTIFIER_READ_VISIBLE_SECTIONS_WHERE_FILTER);
		...
		public Long countVisibleSectionsWhereFilter(QueryExecutorArguments arguments) {
			if(arguments.getQuery() == null)
				arguments.setQueryFromIdentifier(QUERY_IDENTIFIER_COUNT_VISIBLE_SECTIONS_WHERE_FILTER);
			prepareVisibleSectionsWhereFilter(arguments);
		...
		public Collection<Scope> readInvisibleSectionsWhereFilter(QueryExecutorArguments arguments) {
			if(arguments.getQuery() == null)
				arguments.setQueryFromIdentifier(QUERY_IDENTIFIER_READ_INVISIBLE_SECTIONS_WHERE_FILTER);
		...
		public Long countInvisibleSectionsWhereFilter(QueryExecutorArguments arguments) {
			if(arguments.getQuery() == null)
				arguments.setQueryFromIdentifier(QUERY_IDENTIFIER_COUNT_INVISIBLE_SECTIONS_WHERE_FILTER);
			prepareInvisibleSectionsWhereFilter(arguments);
			return QueryExecutor.getInstance().executeCount(arguments);
		}
	...
	static void initialize() {
		...
		QueryHelper.addQueries(Query.build(Query.FIELD_IDENTIFIER,QUERY_IDENTIFIER_READ_VISIBLE_SECTIONS_WHERE_FILTER
				,Query.FIELD_TUPLE_CLASS,Scope.class,Query.FIELD_RESULT_CLASS,Scope.class
				,Query.FIELD_VALUE,"SELECT scope FROM Scope scope " + getQueryValueReadVisibleSectionsWhereFilterWhere()+ " ORDER BY scope.code ASC"
				)
			);
		QueryHelper.addQueries(Query.build(Query.FIELD_IDENTIFIER,QUERY_IDENTIFIER_COUNT_VISIBLE_SECTIONS_WHERE_FILTER
				,Query.FIELD_TUPLE_CLASS,Scope.class,Query.FIELD_RESULT_CLASS,Long.class
				,Query.FIELD_VALUE,"SELECT COUNT(scope.identifier) FROM Scope scope " + getQueryValueReadVisibleSectionsWhereFilterWhere()
				)
			);
		
		QueryHelper.addQueries(Query.build(Query.FIELD_IDENTIFIER,QUERY_IDENTIFIER_READ_INVISIBLE_SECTIONS_WHERE_FILTER
				,Query.FIELD_TUPLE_CLASS,Scope.class,Query.FIELD_RESULT_CLASS,Scope.class
				,Query.FIELD_VALUE,"SELECT scope FROM Scope scope " + getQueryValueReadInvisibleSectionsWhereFilterWhere()+ " ORDER BY scope.code ASC"
				)
			);
		QueryHelper.addQueries(Query.build(Query.FIELD_IDENTIFIER,QUERY_IDENTIFIER_COUNT_INVISIBLE_SECTIONS_WHERE_FILTER
				,Query.FIELD_TUPLE_CLASS,Scope.class,Query.FIELD_RESULT_CLASS,Long.class
				,Query.FIELD_VALUE,"SELECT COUNT(scope.identifier) FROM Scope scope " + getQueryValueReadInvisibleSectionsWhereFilterWhere()
				)
			);
	}
	
	String[] QUERIES_IDENTIFIERS = {
			QUERY_IDENTIFIER_READ_VISIBLE_SECTIONS_BY_ACTOR_CODE,QUERY_IDENTIFIER_COUNT_VISIBLE_SECTIONS_BY_ACTOR_CODE
			,QUERY_IDENTIFIER_READ_VISIBLE_SECTIONS_WHERE_FILTER,QUERY_IDENTIFIER_COUNT_VISIBLE_SECTIONS_WHERE_FILTER
			,QUERY_IDENTIFIER_READ_INVISIBLE_SECTIONS_WHERE_FILTER,QUERY_IDENTIFIER_COUNT_INVISIBLE_SECTIONS_WHERE_FILTER
			};
